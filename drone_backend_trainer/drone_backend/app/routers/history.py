...
def create_history_api(history: HistoryCreate, db: Session = Depends(get_db)):
    """创建历史记录"""
    return create_history(db, history)

@router.get("/", response_model=List[HistoryResponse])
def read_histories_api(db: Session = Depends(get_db)):
    """获取所有历史记录"""
    return get_histories(db)

@router.get("/{history_id}", response_model=HistoryResponse)
def read_history_api(history_id: int, db: Session = Depends(get_db)):
    """根据ID获取历史记录"""
...
def update_history_status_api(history_id: int, new_status: str, db: Session = Depends(get_db)):
    """更新历史记录状态"""
...
def update_history_results_api(history_id: int, results_path: str, db: Session = Depends(get_db)):
...
def delete_history_api(history_id: int, db: Session = Depends(get_db)):
    """删除历史记录"""
